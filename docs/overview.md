...
- Devices are **not stored as a table** while connecting. A `user_devices` row is inserted only after the bot reports `logged_in`. Device references use `device_key` (= the bot service's `device_id`).
...
| API | `/api/webhook/gowa` | Inbound webhook receiver from the bot (HMAC-verified) |
